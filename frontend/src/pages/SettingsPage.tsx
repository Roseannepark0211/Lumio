  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="flex flex-col gap-5 p-6 pb-12">
        ...
        {/* ---------- 已连接设备（移动设备管理） ---------- */}
        {/* 仅在开发模式下显示：终端 `npm run dev` / `electron .` 启动可见；
            构建版本（electron-builder 打包后从 dist 加载）隐藏。
            Vite 在 build 时将 import.meta.env.DEV 静态替换为 false，
            Rollup DCE 移除渲染分支 + tree-shake MobileDevicesSettings 模块，
            构建产物中完全不包含此设置页、相关 API 端点字符串及回调逻辑。 */}
        {import.meta.env.DEV && (
          <MobileDevicesSettings config={config} saveConfig={saveConfig} />
        )}
        ...
        <SettingsCard icon="ℹ" iconColor="text-info" title={tr("about")} desc="Lumio © 2026">
          ...
          {/* 版本号来源说明：config.version 由 /api/config 实时注入 lumio.__version__，
              不持久化到 config.json，确保升级后立即显示新版本号（不会残留旧值） */}
          <p className="text-[10px] text-text-dim">
            {tr("about_version_source")}
          </p>

          <p className="text-center font-mono text-[10px] text-text-dim">
            Lumio © 2026{config.build_date ? ` · Build ${config.build_date}` : ""}
          </p>
        </SettingsCard>
      </div>
